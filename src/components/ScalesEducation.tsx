...
const STEP_LABELS: Record<string,string> = { T:'Whole', S:'Half', A:'Aug' };
      ...
      {activeTab === 'progreso' && (
        <div className="space-y-4">
          ...
          <div className="stat-card">
            <p className="section-title text-sm mb-3">Practice Heatmap · Notes vs Types</p>
            <div className="overflow-x-auto">
              <table className="w-full text-[10px]">
                <thead>
                  <tr>
                    <th className="text-left text-muted-foreground pr-2 font-mono w-14">Note</th>
                    {HEATMAP_TYPES.map(t=>(
                      <th key={t} className="text-muted-foreground px-0.5 pb-1 font-normal">
                        {SCALE_THEORY[t]?.labelEN.split(' ')[0] ?? t}
                      </th>
                    ))}
                  </tr>
                </thead>
                        ...
                        return (
                          <td key={type} className="px-0.5 py-0.5">
                            <div className="w-full h-5 rounded-sm flex items-center justify-center"
                              ...
                              title={c>0 ? `${note} ${SCALE_THEORY[type]?.labelEN}: ${c}×` : ''}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="stat-card">
              <p className="section-title text-sm mb-3">By Scale Type</p>
              <div className="space-y-2">
                {SCALE_TYPE_OPTIONS.filter(t=>byType[t.value]>0).sort((a,b)=>(byType[b.value]||0)-(byType[a.value]||0)).map(t=>(
                  <div key={t.value}>
                    <div className="flex justify-between text-xs mb-0.5">
                      <span style={{color: SCALE_THEORY[t.value]?.color}}>{t.labelEN}</span>
                ...
                {!Object.keys(byType).length && <p className="text-xs text-muted-foreground text-center py-4">No data yet</p>}
              </div>
            </div>
            <div className="stat-card">
              <p className="section-title text-sm mb-3">By Root Note</p>
              <div className="space-y-2">
                ...
                {!Object.keys(byNote).length && <p className="text-xs text-muted-foreground text-center py-4">No data yet</p>}
              </div>
            </div>
          </div>
      ...
      {activeTab === 'teoria' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="stat-card">
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-2">Scale Type</p>
              <select value={theoryType} onChange={e=>setTheoryType(e.target.value)}
                ...
                {SCALE_TYPE_OPTIONS.map(t=><option key={t.value} value={t.value}>{t.labelEN}</option>)}
              </select>
            </div>
            <div className="stat-card">
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-2">Root Note</p>
            ...
            <>
              ...
              <div className="stat-card" style={{borderColor: theory.color+'44', background: theory.color+'0a'}}>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="font-bold text-lg">{rootSimple} {theory.labelEN}</h3>
                </div>
              ...
              {/* Interval steps */}
              <div className="stat-card">
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-3">Interval Pattern</p>
                ...
                <div className="flex gap-3 mt-3 text-[10px] text-muted-foreground">
                  <span><span className="inline-block w-2 h-2 rounded bg-primary/70 mr-1" />T = Whole step (2 semitones)</span>
                  <span><span className="inline-block w-2 h-2 rounded bg-amber-400/80 mr-1" />S = Half step (1 semitone)</span>
                  <span><span className="inline-block w-2 h-2 rounded bg-purple-400/80 mr-1" />A = Aug 2nd (3 semitones)</span>
                </div>
              </div>
              ...
              <div className="stat-card">
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-3">
                  Notes of {rootSimple} {theory.labelEN}
                </p>
